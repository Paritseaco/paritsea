...
import { f as union, p as literal, o as object$1, x as unknown, h as string, n as number, j as _enum$1, e as array, g4 as optional, g5 as preprocess, r as record, k as boolean, g6 as looseObject, l as discriminatedUnion, g7 as _null, ar as custom, g8 as intersection, g9 as datetime, ga as normalizeParams, gb as $constructor, gc as $ZodObject, gd as defineLazy, ge as $ZodType, gf as parse, gg as safeParse$1, gh as parseAsync, gi as safeParseAsync$1, gj as clone, gk as toJSONSchema, ap as getDefaultExportFromCjs, gl as ZodOptional$1 } from "./index_BpQCdXij.mjs";
...
async function safeParseAsync(schema, data) {
  if (isZ4Schema(schema)) {
    const result2 = await safeParseAsync$1(schema, data);
    return result2;
  }
  const v3Schema = schema;
...
function unwrap(result) {
  ...
  const errMsg = result.error && typeof result.error === "object" && "message" in result.error ? String(result.error.message) : "Unknown error";
...
function requireScope(extra, scope2) {
  const payload = getExtra(extra);
  if (payload.tokenScopes && !hasScope(payload.tokenScopes, scope2)) {
    throw new McpError(ErrorCode.InvalidRequest, `Insufficient scope: requires ${scope2}`);
  }
}
...
function createMcpServer() {
  ...
  server.registerTool(
    "content_list",
    {
      ...
      inputSchema: object$1({
        collection: string().describe("Collection slug (e.g. 'posts', 'pages')"),
        status: _enum$1(["draft", "published", "scheduled"]).optional().describe("Filter by content status"),
        limit: number().int().min(1).max(100).optional().describe("Max items to return (default 50, max 100)"),
        cursor: string().optional().describe("Pagination cursor from a previous response"),
        orderBy: string().optional().describe("Field to sort by (e.g. 'created_at', 'updated_at')"),
        order: _enum$1(["asc", "desc"]).optional().describe("Sort direction (default 'desc')"),
        locale: string().optional().describe("Filter by locale (e.g. 'en', 'fr'). Only relevant when i18n is enabled.")
      }),
  ...
  server.registerTool(
    "content_get",
    {
      ...
      inputSchema: object$1({
        collection: string().describe("Collection slug (e.g. 'posts', 'pages')"),
        id: string().describe("Content item ID (ULID) or slug"),
        locale: string().optional().describe(
  ...
  server.registerTool(
    "content_create",
    {
      ...
      inputSchema: object$1({
        collection: string().describe("Collection slug (e.g. 'posts', 'pages')"),
        data: record(string(), unknown()).describe("Field values as key-value pairs matching the collection schema"),
        slug: string().optional().describe("URL slug (auto-generated from title if omitted)"),
        status: _enum$1(["draft", "published"]).optional().describe("Initial status (default 'draft'). Requires publish permission."),
        locale: string().optional().describe("Locale for this content (e.g. 'fr'). Defaults to default locale."),
        translationOf: string().optional().describe(
  ...
  server.registerTool(
    "content_update",
    {
      ...
      description: "Update an existing content item. Only include fields you want to change in the 'data' object — unspecified fields are left unchanged. Pass the _rev token from content_get to enable optimistic concurrency checking (the update fails if the item was modified since you read it).",
      inputSchema: object$1({
        collection: string().describe("Collection slug"),
        id: string().describe("Content item ID or slug"),
        data: record(string(), unknown()).optional().describe("Field values to update (only include changed fields)"),
        slug: string().optional().describe("New URL slug"),
        status: _enum$1(["draft", "published"]).optional().describe(
        ...
        _rev: string().optional().describe("Revision token from content_get for conflict detection")
      })
    },
    async (args, extra) => {
      requireScope(extra, "content:write");
  ...
  server.registerTool(
    "content_publish",
    {
      ...
      description: "Publish a content item, making it live on the site. Creates a published revision from the current draft. Further edits create a new draft without affecting the live version until re-published.",
      inputSchema: object$1({
        collection: string().describe("Collection slug"),
        id: string().describe("Content item ID or slug")
      })
  ...
  server.registerTool(
    "content_schedule",
    {
      ...
      inputSchema: object$1({
        collection: string().describe("Collection slug"),
        id: string().describe("Content item ID or slug"),
        scheduledAt: string().describe("ISO 8601 datetime for publication (e.g. '2025-06-01T09:00:00Z')")
      })
    },
    async (args, extra) => {
      requireScope(extra, "content:write");
  ...
  server.registerTool(
    "content_list_trashed",
    {
      ...
      inputSchema: object$1({
        collection: string().describe("Collection slug"),
        limit: number().int().min(1).max(100).optional().describe("Max items (default 50)"),
        cursor: string().optional().describe("Pagination cursor")
      }),
      annotations: { readOnlyHint: true }
    },
    async (args, extra) => {
  ...
  server.registerTool(
    "content_duplicate",
    {
      ...
      inputSchema: object$1({
        collection: string().describe("Collection slug"),
        id: string().describe("Content item ID or slug to duplicate")
      })
    },
    async (args, extra) => {
      requireScope(extra, "content:write");
  ...
  server.registerTool(
    ...
    async (_args, extra) => {
      ...
      const ec = getEmDash(extra);
      try {
        const { SchemaRegistry } = await import("./index_b5jYPigD.mjs");
        const registry = new SchemaRegistry(ec.db);
        const items2 = await registry.listCollections();
        return jsonResult({ items: items2 });
      } catch (error) {
  ...
  server.registerTool(
    "schema_create_collection",
    {
      ...
      inputSchema: object$1({
        slug: string().regex(COLLECTION_SLUG_PATTERN).describe("Unique identifier (lowercase letters, numbers, underscores)"),
        label: string().describe("Display name (plural, e.g. 'Blog Posts')"),
        labelSingular: string().optional().describe("Singular display name (e.g. 'Blog Post')"),
        description: string().optional().describe("Description of this collection"),
        icon: string().optional().describe("Icon name for the admin UI"),
        supports: array(_enum$1(["drafts", "revisions", "preview", "scheduling", "search"])).optional().describe("Features to enable (default: ['drafts', 'revisions'])")
      })
    },
    async (args, extra) => {
  ...
  server.registerTool(
    "schema_delete_collection",
    {
      ...
      inputSchema: object$1({
        slug: string().describe("Collection slug to delete"),
        force: boolean().optional().describe("Force deletion even if the collection has content (default false)")
      }),
      annotations: { destructiveHint: true }
    },
    async (args, extra) => {
  ...
  server.registerTool(
    "schema_create_field",
    {
      ...
      inputSchema: object$1({
        collection: string().describe("Collection slug to add the field to"),
        slug: string().regex(COLLECTION_SLUG_PATTERN).describe("Field identifier (lowercase letters, numbers, underscores)"),
        label: string().describe("Display name for the field"),
        type: _enum$1([
          "string",
          "text",
          "number",
        ...
        required: boolean().optional().describe("Whether the field is required (default false)"),
        unique: boolean().optional().describe("Whether values must be unique (default false)"),
        defaultValue: unknown().optional().describe("Default value for new items"),
        validation: object$1({
          min: number().optional(),
          max: number().optional(),
          minLength: number().optional(),
          ...
          options: array(string()).optional().describe("Allowed values for select/multiSelect")
        }).optional().describe("Validation constraints"),
        options: object$1({
          collection: string().optional().describe("Target collection slug for reference fields"),
          rows: number().optional().describe("Number of rows for textarea")
        }).passthrough().optional().describe("Widget configuration"),
        searchable: boolean().optional().describe("Include in full-text search index (default false)"),
        translatable: boolean().optional().describe(
  ...
  server.registerTool(
    "schema_delete_field",
    {
      ...
      inputSchema: object$1({
        collection: string().describe("Collection slug"),
        fieldSlug: string().describe("Field slug to remove")
      }),
      annotations: { destructiveHint: true }
    },
    async (args, extra) => {
  ...
  server.registerTool(
    "media_list",
    {
      ...
      inputSchema: object$1({
        mimeType: string().optional().describe("Filter by MIME type prefix (e.g. 'image/', 'application/pdf')"),
        limit: number().int().min(1).max(100).optional().describe("Max items (default 50)"),
        cursor: string().optional().describe("Pagination cursor")
      }),
      annotations: { readOnlyHint: true }
  ...
  server.registerTool(
    "media_get",
    {
      ...
      inputSchema: object$1({
        id: string().describe("Media item ID")
      }),
      annotations: { readOnlyHint: true }
    },
    async (args, extra) => {
  ...
  server.registerTool(
    "media_update",
    {
      ...
      inputSchema: object$1({
        id: string().describe("Media item ID"),
        alt: string().optional().describe("Alt text for accessibility"),
        caption: string().optional().describe("Caption text"),
        width: number().int().optional().describe("Image width in pixels"),
        height: number().int().optional().describe("Image height in pixels")
      })
    },
    async (args, extra) => {
  ...
  server.registerTool(
    "search",
    {
      ...
      inputSchema: object$1({
        query: string().describe("Search query text"),
        collections: array(string()).optional().describe("Limit search to specific collection slugs (all if omitted)"),
        locale: string().optional().describe("Filter results by locale (omit to search all locales)"),
        limit: number().int().min(1).max(50).optional().describe("Max results (default 20)")
      }),
      annotations: { readOnlyHint: true }
    },
    async (args, extra) => {
      ...
      const ec = getEmDash(extra);
      try {
        const { searchWithDb } = await import("./index_CTqqT_9c.mjs");
  ...
  server.registerTool(
    ...
    async (_args, extra) => {
      ...
      const ec = getEmDash(extra);
      try {
        const { handleTaxonomyList } = await import("./taxonomies_DNC4r8fq.mjs");
  ...
  server.registerTool(
    "taxonomy_list_terms",
    {
      ...
      inputSchema: object$1({
        taxonomy: string().describe("Taxonomy name (e.g. 'categories', 'tags')"),
        limit: number().int().min(1).max(100).optional().describe("Max items (default 50)"),
        cursor: string().optional().describe("Pagination cursor")
      }),
      annotations: { readOnlyHint: true }
    ...
    async (args, extra) => {
      ...
      try {
        ...
        const { TaxonomyRepository } = await import("./taxonomy_NGzK5W_h.mjs");
        const repo = new TaxonomyRepository(ec.db);
        const limit2 = Math.min(args.limit ?? 50, 100);
        ...
        const page2 = terms.slice(startIdx, startIdx + limit2);
        const hasMore = startIdx + limit2 < terms.length;
        const nextCursor = hasMore ? page2.at(-1)?.id : void 0;
        return jsonResult({
          items: page2.map((t) => ({
            id: t.id,
            name: t.name,
  ...
  server.registerTool(
    "taxonomy_create_term",
    {
      ...
      inputSchema: object$1({
        taxonomy: string().describe("Taxonomy name (e.g. 'categories', 'tags')"),
        slug: string().describe("URL-safe identifier for the term"),
        label: string().describe("Display name"),
        parentId: string().optional().describe("Parent term ID for hierarchical taxonomies"),
        description: string().optional().describe("Description of the term")
      })
    },
    async (args, extra) => {
      requireScope(extra, "content:write");
      ...
      const ec = getEmDash(extra);
      try {
        const { handleTermCreate } = await import("./taxonomies_DNC4r8fq.mjs");
        return unwrap(
  ...
  server.registerTool(
    ...
    async (_args, extra) => {
      ...
      const ec = getEmDash(extra);
      try {
        const menus = await ec.db.selectFrom("_emdash_menus").select([
          "id",
          "name",
          "label",
          "created_at",
  ...
  server.registerTool(
    "menu_get",
    {
      ...
      inputSchema: object$1({
        name: string().describe("Menu name (e.g. 'main', 'footer')")
      }),
      annotations: { readOnlyHint: true }
    },
    async (args, extra) => {
      ...
      const ec = getEmDash(extra);
      try {
        const menu = await ec.db.selectFrom("_emdash_menus").selectAll().where("name", "=", args.name).executeTakeFirst();
        if (!menu) return errorResult(`Menu '${args.name}' not found`);
        const items2 = await ec.db.selectFrom("_emdash_menu_items").selectAll().where("menu_id", "=", menu.id).orderBy("sort_order", "asc").execute();
        return jsonResult({ ...menu, items: items2 });
      } catch (error) {
        return errorResult(error);
      }
    }
  ...
  server.registerTool(
    "revision_list",
    {
      ...
      inputSchema: object$1({
        collection: string().describe("Collection slug"),
        id: string().describe("Content item ID or slug"),
        limit: number().int().min(1).max(50).optional().describe("Max revisions (default 20)")
      }),
      annotations: { readOnlyHint: true }
    },
    async (args, extra) => {
  ...
  server.registerTool(
    "revision_restore",
    {
      ...
      inputSchema: object$1({
        revisionId: string().describe("Revision ID to restore")
      })
    },
    async (args, extra) => {
      requireScope(extra, "content:write");
