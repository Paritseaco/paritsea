...
const wordpressPluginSource = {
  ...
  async probe(url) {
    try {
      ...
      validateExternalUrl(siteUrl);
      const probeUrl2 = `${siteUrl}/wp-json/emdash/v1/probe`;
      const response = await ssrfSafeFetch(probeUrl2, {
        headers: { Accept: "application/json" },
        signal: AbortSignal.timeout(1e4)
      });
      if (!response.ok) {
